...
namespace NzbDrone.Test.Common
{
    public class NzbDroneRunner
    {
        ...
        public NzbDroneRunner(Logger logger, PostgresOptions postgresOptions, int port = 9898)
        {
        ...
        public void Start(bool enableAuth = false)
        {
            ...
            if (OsInfo.IsWindows)
            {
                consoleExe = "Releasarr.Console.exe";
            }
            else
            {
                consoleExe = "Releasarr";
            }
            ...
            while (true)
            {
                ...
                {
                    TestContext.Progress.WriteLine($"Releasarr {Port} is started. Running Tests");
                    return;
                }

                TestContext.Progress.WriteLine("Waiting for Releasarr to start. Response Status : {0}  [{1}] {2}", statusCall.ResponseStatus, statusCall.StatusDescription, statusCall.ErrorException.Message);
        ...
        public void Kill()
        {
            try
            {
                if (_nzbDroneProcess != null)
                {
                    ...
                    if (_nzbDroneProcess.HasExited)
                    {
                        var log = File.ReadAllLines(Path.Combine(AppData, "logs", "Releasarr.trace.txt"));
        ...
        public void KillAll()
        {
            try
            {
                ...
                _processProvider.KillAll(ProcessProvider.RELEASARR_CONSOLE_PROCESS_NAME);
                _processProvider.KillAll(ProcessProvider.RELEASARR_PROCESS_NAME);
            }
            catch (InvalidOperationException)
            {
                // May happen if the process closes while being closed
        ...
        private void Start(string outputReleasarrConsoleExe)
        {
            StringDictionary envVars = new();
            if (PostgresOptions?.Host != null)
            {
                envVars.Add("Releasarr__Postgres__Host", PostgresOptions.Host);
                envVars.Add("Releasarr__Postgres__Port", PostgresOptions.Port.ToString());
                envVars.Add("Releasarr__Postgres__User", PostgresOptions.User);
                envVars.Add("Releasarr__Postgres__Password", PostgresOptions.Password);
                envVars.Add("Releasarr__Postgres__MainDb", PostgresOptions.MainDb);
                envVars.Add("Releasarr__Postgres__LogDb", PostgresOptions.LogDb);

                TestContext.Progress.WriteLine("Using env vars:\n{0}", envVars.ToJson());
            }

            TestContext.Progress.WriteLine("Starting instance from {0} on port {1}", outputReleasarrConsoleExe, Port);

            var args = "-nobrowser -nosingleinstancecheck -data=\"" + AppData + "\"";
            _nzbDroneProcess = _processProvider.Start(outputReleasarrConsoleExe, args, envVars, OnOutputDataReceived, OnOutputDataReceived);
        }
