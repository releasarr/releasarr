...
namespace NzbDrone.Host
{
    ...
    public class SingleInstancePolicy : ISingleInstancePolicy
    {
        ...
        public void PreventStartIfAlreadyRunning()
        {
            if (IsAlreadyRunning())
            {
                _logger.Warn("Another instance of Releasarr is already running.");
        ...
        public void WarnIfAlreadyRunning()
        {
            if (IsAlreadyRunning())
            {
                _logger.Debug("Another instance of Releasarr is already running.");
            }
        }
        ...
        private List<int> GetOtherNzbDroneProcessIds()
        {
            try
            {
                ...
                var otherProcesses = _processProvider.FindProcessByName(ProcessProvider.RELEASARR_CONSOLE_PROCESS_NAME)
                                                     .Union(_processProvider.FindProcessByName(ProcessProvider.RELEASARR_PROCESS_NAME))
                                                     .Select(c => c.Id)
                ...
                {
                    _logger.Info("{0} instance(s) of Releasarr are running", otherProcesses.Count);
                }
            ...
            {
                _logger.Warn(ex, "Failed to check for multiple instances of Releasarr.");
                return new List<int>();
            }
        }
    }
